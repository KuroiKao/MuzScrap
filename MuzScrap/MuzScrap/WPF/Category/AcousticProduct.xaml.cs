...
{
    /// <summary>
    /// Логика взаимодействия для AcousticProduct.xaml
    /// </summary>
    public partial class AcousticProduct : Window
    {
        private readonly UserIdNow _userID = new(default);
        public AcousticProduct(UserIdNow userID)
        {
            InitializeComponent();
            _userID = userID;
            ListProduct.ItemsSource = MuzScrapBdContext.GetInstance().Products.Where(x => x.ProductType.Contains("Акустические")).ToList();
            TotalStrings.Text = MuzScrapBdContext.GetInstance().Products.Where(x => x.ProductType.Contains("Акустические")).Count().ToString();

        }
        private void SearchBox_KeyUp(object sender, System.Windows.Input.KeyEventArgs e)
        {
            var filtered = MuzScrapBdContext.GetInstance().Products.ToList().Where(search =>
                (search.ProductType.Contains("Акустические"))
