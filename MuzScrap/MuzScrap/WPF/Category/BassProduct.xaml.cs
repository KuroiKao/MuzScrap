...
{
    /// <summary>
    /// Логика взаимодействия для BassProduct.xaml
    /// </summary>
    public partial class BassProduct : Window
    {
        private readonly UserIdNow _userID = new(default);
        public BassProduct(UserIdNow userID)
        {
            InitializeComponent();
            _userID = userID;
            ListProduct.ItemsSource = MuzScrapBdContext.GetInstance().Products.Where(x => x.ProductType.Contains("Бас")).ToList();
            TotalStrings.Text = MuzScrapBdContext.GetInstance().Products.Where(x => x.ProductType.Contains("Бас")).Count().ToString();
        }
        private void SearchBox_KeyUp(object sender, System.Windows.Input.KeyEventArgs e)
        {
            var filtered = MuzScrapBdContext.GetInstance().Products.ToList().Where(search =>
                (search.ProductType.Contains("Бас"))
